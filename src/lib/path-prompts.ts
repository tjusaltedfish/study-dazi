...
返回 JSON（严格按格式，不要其他文字），包含 4-8 个一级学习阶段。

每个阶段：{ id: string, title: string, description: string, estimated_hours: number, is_required: boolean, why: string }
...
返回 JSON（严格按格式，不要其他文字），包含 3-6 个子节点。

每个子节点：{ id: string, title: string, description: string, estimated_hours: number, node_type: "required"|"optional"|"advanced", resources_hint: string, check_criteria: string }
